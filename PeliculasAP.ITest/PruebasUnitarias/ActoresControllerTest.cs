...
namespace PeliculasAP.ITest.PruebasUnitarias
{
    ...
    public class ActoresControllerTest : BasePrueba
    {
        ...
        public async Task ObtenerPersonasPaginadas()
        {
            ...
            var pagina1 = await controller.Get(new PaginacionDTO() { Pagina = 1, CantidadDeRegistrosPorPagina = 2 });
            var actoresPagina1 = pagina1.Value;
            Assert.AreEqual(2, actoresPagina1.Count);

            controller.ControllerContext.HttpContext = new DefaultHttpContext();

            var pagina2 = await controller.Get(new PaginacionDTO() { Pagina = 2, CantidadDeRegistrosPorPagina = 2 });
            var actoresPagina2 = pagina2.Value;
            Assert.AreEqual(1, actoresPagina2.Count);

            controller.ControllerContext.HttpContext = new DefaultHttpContext();

            var pagina3 = await controller.Get(new PaginacionDTO() { Pagina = 3, CantidadDeRegistrosPorPagina = 2 });
